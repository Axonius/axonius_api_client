...
class Wizard:
    ...
    def _parse_entries(self, entries: List[dict], source: str) -> List[dict]:
        ...
        for idx, entry in enumerate(entries):
            ...
            entry = self._check_entry(entry=entry)
    ...
    def _parse_exprs(self, entries: List[dict]) -> List[dict]:
        ...
        for idx, entry in enumerate(entries):
            try:
                ...
                err = f"Error parsing from {entry[Entry.SRC]}:\n{exc}"
    ...
    def _split_flags(
        ...
        match = pattern.search(value_raw)
        self.LOG.debug(
            f"Value {value_raw!r} regex match {match} using pattern {pattern.pattern}"
        )
    ...
    def _check_entry(self, entry: dict) -> dict:
        try:
            self._check_entry_keys(entry=entry, keys=Entry.REQ)
            etype = self._check_entry_type(entry=entry, types=Types.DICT)
            flags, value = self._split_flags(
                value_raw=entry[Entry.VALUE], flags=listify(entry.get(Entry.FLAGS) or [])
            )
