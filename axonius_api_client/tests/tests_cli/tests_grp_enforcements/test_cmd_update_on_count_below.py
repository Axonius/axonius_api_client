...
class TestGrpEnforcementsCmdUpdateOnCountBelow(EnforcementsBase):
    def test_success(self, request, monkeypatch, apiobj, created_set_trigger):
        runner = load_clirunner(request, monkeypatch)
        with runner.isolated_filesystem():
            args = [
                "enforcements",
                "update-on-count-below",
                "--value",
            ...
            assert "On Count Below: 5" in result.stdout
    ...
    def test_none(self, request, monkeypatch, apiobj, created_set_trigger):
        ...
        with runner.isolated_filesystem():
            ...
            assert result.stderr
            assert result.exit_code == 0
            assert "On Count Below: None" in result.stdout
