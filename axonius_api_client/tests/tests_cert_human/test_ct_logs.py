...
class TestLoad:
    ...
    def test_fallback(self, tmp_path):
        ...
        path.mkdir()
        data = ct_logs.load(path=path, url="192.168.1.100", timeout=0.1)
        check_data(data)
...
class TestFileInfo:
    def test_calc_refetch_exists(self, tmp_path):
        ...
        assert repr(finfo)
        assert finfo.exists
        assert finfo.is_past_modified_days_max is False
        ...
        refetch = finfo.calc_refetch(refetch=False)
    ...
    def test_calc_refetch_exists_past_max_days(self, tmp_path):
        path = tmp_path / "badwolf.json"
        path.touch()
        finfo = ct_logs.FileInfo(path=path, modified_days_max=-1)

        assert str(finfo)
        assert repr(finfo)
        assert finfo.exists is True
        assert finfo.is_past_modified_days_max is True
        assert isinstance(finfo.modified_dt, datetime.datetime)
        assert isinstance(finfo.modified_delta, datetime.timedelta)
        assert isinstance(finfo.modified_days, int)
