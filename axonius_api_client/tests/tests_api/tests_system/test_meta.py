...
class TestSystemMetaPrivate(SystemMetaBase):
    ...
    def val_historical_sizes(self, data):
        ...
        assert isinstance(devices, dict) or devices is None
...
class TestSystemMetaPublic(SystemMetaBase):
    ...
    def test_historical_sizes(self, apiobj):
        ...
        disk_free = data.pop("disk_free_mb")
        disk_used = data.pop("disk_used_mb")
        hist_devices = data.pop("historical_sizes_devices", {})
        hist_users = data.pop("historical_sizes_users", {})

        assert isinstance(disk_free, float)
        assert isinstance(disk_used, float)
        assert isinstance(hist_devices, dict) or hist_devices is None
        assert isinstance(hist_users, dict) or hist_users is None
        assert not data
