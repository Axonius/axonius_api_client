...
class TestCnxBase:
    ...
    def apiobj(self, api_client):
        return api_client.cnx

    @pytest.fixture(scope="class")
    def adapter(self, apiobj):
        return apiobj.CLIENT.adapters.get_by_name(name=CSV_ADAPTER, get_clients=False)
...
class TestCnxPublic(TestCnxBase):
    def test_get_by_adapter(self, apiobj):
        cnxs = apiobj.get_by_adapter(adapter_name=CSV_ADAPTER)
    ...
    def test_get_by_adapter_badname(self, apiobj):
        with pytest.raises(NotFoundError):
            apiobj.get_by_adapter(adapter_name="badwolf")
    ...
    def test_update_cnx(self, apiobj):
        ...
        cnx_update = apiobj.update_cnx(cnx_update=cnx, **{config_key: value_to_set})
        assert cnx_update["config"][config_key] == value_to_set

        cnx_reset = apiobj.update_by_id(
        ...
        with pytest.raises(CnxGoneError):
            apiobj.update_cnx(cnx_update=cnx, **{config_key: 9999})

        cnx_final = apiobj.get_by_id(cnx_id=cnx_reset["id"], adapter_name=cnx_reset["adapter_name"])
    ...
    def test_update_cnx_error(self, apiobj):
        ...
        with pytest.raises(CnxUpdateError) as exc:
            apiobj.update_cnx(cnx_update=cnx, **{config_key: value_to_set})
        ...
        cnx_reset = apiobj.update_cnx(cnx_update=exc.value.cnx_new, **{config_key: value_orig})
        assert cnx_reset["config"][config_key] == value_orig
