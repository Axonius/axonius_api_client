...
class TestCnxPublic(TestCnxBase):
    ...
    def test_update_cnx_error(self, api_client):
        config_key = "https_proxy"

        cnx = get_cnx_working(api_client=api_client, reqkeys=[config_key])
        ...
        with pytest.raises(CnxUpdateError) as exc:
            api_client.cnx.update_cnx(cnx_update=cnx, **{config_key: value_to_set})
        ...
        cnx_reset = api_client.cnx.update_cnx(
