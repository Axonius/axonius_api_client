...
class TestAuthUser(object):
    ...
    @pytest.mark.needs_user_creds
    def test_validate_not_logged_in(self, api_url, creds_user):
        """Test exc thrown when validate() but login() not called."""
        creds = {k: v for k, v in creds_user.items() if k != "cls"}
        http_client = axonius_api_client.http.HttpClient(url=api_url)
        auth = self.auth_cls(http_client=http_client, **creds)
        with pytest.raises(axonius_api_client.exceptions.NotLoggedIn):
    ...
    def test_login_already_logged_in(self, api_url, creds_user):
        ...
        auth = self.auth_cls(http_client=http_client, **creds)
        auth.login()
        with pytest.raises(axonius_api_client.exceptions.AlreadyLoggedIn):
            auth.login()
...
class TestAuthKey(object):
    ...
    @pytest.mark.needs_key_creds
    def test_logout(self, api_url, creds_key):
        """Test no exc when logout() after login()."""
        creds = {k: v for k, v in creds_key.items() if k != "cls"}
        http_client = axonius_api_client.http.HttpClient(url=api_url)
        auth = self.auth_cls(http_client=http_client, **creds)
    ...
    @pytest.mark.needs_key_creds
    def test_login_already_logged_in(self, api_url, creds_key):
        """Test exc thrown when login() and login() already called."""
        creds = {k: v for k, v in creds_key.items() if k != "cls"}
        http_client = axonius_api_client.http.HttpClient(url=api_url)
        auth = self.auth_cls(http_client=http_client, **creds)
    ...
    @pytest.mark.needs_key_creds
    def test_logout_not_logged_in(self, api_url, creds_key):
        """Test exc thrown when logout() but login() not called."""
        creds = {k: v for k, v in creds_key.items() if k != "cls"}
        http_client = axonius_api_client.http.HttpClient(url=api_url)
        auth = self.auth_cls(http_client=http_client, **creds)
    ...
    @pytest.mark.needs_key_creds
    def test_validate_not_logged_in(self, api_url, creds_key):
        """Test exc thrown when validate() but login() not called."""
        creds = {k: v for k, v in creds_key.items() if k != "cls"}
        http_client = axonius_api_client.http.HttpClient(url=api_url)
        auth = self.auth_cls(http_client=http_client, **creds)
