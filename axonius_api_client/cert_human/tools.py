...
HUMAN_CAP_WORDS: List[str] = ["id", "url", "crl", "sha256", "sha1"]


def type_str(value: Any, max_len: int = 40) -> str:
    """Pass."""
    length = len(str(value))
    svalue = str(value)[:max_len] if length >= max_len else f"{value}"
    return f"type={type(value)}, length={length}, value={svalue!r}"
...
def human_key_value(key: str, value: str, indent: int = 2) -> str:
    ...
    return f"{pre(k=human_key(key)):<{HUMAN_KEY_ALIGN}}: {value}"
...
def human_dict(value: dict, indent: int = 2) -> List[str]:
    ...
    for k, v in value.items():
        if isinstance(v, list) and all([isinstance(x, dict) for x in v]):
            for sub_v in v:
                items += human_dict(value=sub_v, indent=indent + 2)
        else:
            items.append(human_key_value(key=k, value=v, indent=indent))
    return items
...
def bytes_to_str(value: Union[str, bytes], strict: bool = True) -> bytes:
    """Coerce bytes into str."""
    check_type(value=value, exp=(str, bytes))
    errors = "strict" if strict else "xmlcharrefreplace"
    return value.decode("utf-8", errors) if isinstance(value, bytes) else value
...
def b64_to_hex(value: Union[str, bytes]) -> str:
    """Pass."""
    return bytes_to_hex(value=b64_to_bytes(value=value))


def b64_to_bytes(value: Union[str, bytes]) -> bytes:
    """Pass."""
    check_type(value=value, exp=(str, bytes))
    return base64.b64decode(str_to_bytes(value=value))
...
def str_to_bytes(value: Union[str, bytes], strict: bool = True) -> bytes:
    """Coerce string into bytes."""
    check_type(value=value, exp=(str, bytes))
    errors = "strict" if strict else "xmlcharrefreplace"
    return value.encode("utf-8", errors) if isinstance(value, str) else value
