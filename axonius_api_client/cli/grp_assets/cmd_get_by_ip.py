...
@click.command(name="get-by-ip", context_settings=cli_constants.CONTEXT_SETTINGS)
@options.OPT_URL
@options.OPT_KEY
@options.OPT_SECRET
...
@click.pass_context
def cmd(ctx, **kwargs):
    """Get assets with matching IP addresses."""
    grp_common.get_by_cmd(ctx=ctx, method="get_by_ip", **kwargs)
