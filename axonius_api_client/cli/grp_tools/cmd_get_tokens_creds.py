...
@click.command(name="get-tokens-creds", context_settings=CONTEXT_SETTINGS)
@add_options(OPTIONS)
@click.pass_context
def cmd(ctx, url, user_name, password, env_file, write_env, config, export_format):
    """Get API tokens using username and password."""
    client = ctx.obj.get_client(url=url)

    with ctx.obj.exc_wrap(wraperror=ctx.obj.wraperror):
        data = client.signup.get_api_keys(user_name=user_name, password=password)
